...
local function new(self, major_version)
  ...
  local function send(status, body, headers)
    ...
    if json ~= nil then
      ...
      if self.ctx.core.phase == PHASES.header_filter then
  ...
  function _RESPONSE.exit(status, body, headers)
    local is_buffered_exit = self.ctx.core.buffered_proxying and
                             self.ctx.core.phase == PHASES.balancer and
                             ngx.get_phase()     == "access"
